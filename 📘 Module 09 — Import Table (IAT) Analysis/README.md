...
✔ Import Table = Static description
✔ IAT = Runtime table with real addresses
🧠 Reading the Import Directory in CFF Explorer
...
📸 Screenshot — Import Directory (CFF Explorer):

From the screenshot:

USER32.dll → 9 functions
...
🧠 Understanding USER32.dll Imports
...
(Using Xrefs — Cross References)

Example:
When you check MessageBoxA references, IDA shows all code locations calling it.

📸 Screenshot — Xrefs to MessageBoxA:

This reveals:

Success message block (“Congrats!”)

Failure message block (“Wrong serial”)
...
If the call operand is inside .idata, it’s an API call.

🧪 Exercises (Required)
Exercise A — CFF Explorer
